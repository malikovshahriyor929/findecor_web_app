        ...
        {currentItems.map((value: ProductType, i: number) => {
          return (
            <div key={i} className="relative w-full ">
              <div
                className={`  flex items-center flex-col bg-white p-2 max-[1300px]:p-2 rounded-lg shadow-[0_1px_2px_-0px_rgba(0,0,0,0.08),inset_0_-0px_0_0_rgba(0,0,0,0.12)] gap-4  ${
                  loading && "blur-sm "
                }  `}
              >
                <Image
            ...
            {getPaginationItems().map((number, i) => (
              <PaginationItem key={i}>
                <button
                  ...
                  className={`px-3 py-1 rounded cursor-pointer max-[370px]:px-2 ${
                    currentPage === number
