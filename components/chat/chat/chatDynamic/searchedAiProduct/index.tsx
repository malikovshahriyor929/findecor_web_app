...
const SearchProduct = ({ message }: { message: ChatMessageType }) => {
  // ____redux____
  ...
  const totalPages = Math.ceil(
    JSON.parse(message.content)[0].products?.length / itemsPerPage
  );
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = JSON.parse(message.content)[0].products.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
  ...
  return (
    <div>
      <p className="text-sm font-medium px-1 ">
        Here are some products similar to the objects you selected:
      </p>
      <div className="flex items-center justify-between py-5 px-1 w-[]">
        <div className="flex relative items-center gap-2 max-[480px]:gap-1 ">
          <Image
            src={
            ...
            width={ 60 }
            height={ 60 }
            className="rounded-lg  max-h-[50px] min-w-[50px]  !object-cover"
            alt={ "wa" }
          />
        </div>
        { JSON.parse(message.content)[0].products.length > 3 && (
          <Button
            variant={ "outline" }
            onClick={ () => {
              setPagOpen(!pagOpen);
          ...
          >
            { pagOpen ? (
              <p className="">
                Hide <span className="max-[460px]:hidden">products</span>
              </p>
            ) : (
              <p
                onClick={ () => setCurrentPage(2) }
                className="max-[460px]:capitalize "
              >
                <span className="max-[460px]:hidden">Show</span> more
              </p>
            ) }
          </Button>
        ) }
        ...
        { currentItems?.map((value: ProductType, i: number) => {
          return (
            <div key={ i } className="relative w-full ">
              <div
                className={ `flex items-center flex-col bg-white p-2 rounded-lg shadow-[0_1px_2px_-0px_rgba(0,0,0,0.08),inset_0_-0px_0_0_rgba(0,0,0,0.12)] gap-4  max-[1382px]:p-2 relative w-full  ${loading && "blur-sm "
                  }   ` }
              >
                <Image
                  src={
                  ...
                  height={ 120 }
                  alt={ value.name }
                  className="!rounded-lg !size-30  max-[454px]:!size-40 object-contain max-[387px]:!size-40  "
                  loading="eager"
                />
                <div className="flex items-center gap-2 max-[453px]:gap-4 w-full justify-center">
                  <button
                    onClick={ () => {
                    ...
                    className="!bg-[#ea580b] cursor-pointer border-[rgba(10,10,10,0.16)] border-[0.5px] text-white font-medium  text-sm rounded-md px-2 py-1 flex items-center gap-1 text-nowrap shadow-[0_1px_2px_-0px_rgba(0,0,0,0.08),inset_0_-0px_0_0_rgba(0,0,0,0.12)] max-[454px]:!w-full max-[454px]:py-1.5 max-[454px]:px-18 max-[385px]:px-15 max-[358px]:px-10 w-full justify-center"
                  >
                    View <MdArrowOutward />
                  </button>
                  <div className="max-[454px]:absolute top-2 right-2  max-[454px]:*:rounded-full max-[454px]:*:p-1.5 ">
            ...
            <PaginationItem>
              <button
                ...
                className={ `${currentPage === 1 ? "pointer-events-none  opacity-40 " : ""
                  } my-auto bg-white px-3 py-2 border cursor-pointer border-gray-200 rounded-sm mr-2 max-[312px]:px-2 max-[312px]:mr-1` }
              >
                <IoIosArrowBack />
              </button>
            </PaginationItem>
            ...
            <PaginationItem>
              <button
                className={ `${currentPage === totalPages
                  ...
                  : ""
                  } my-auto bg-white px-3 py-2 border cursor-pointer border-gray-200 rounded-sm ml-2 max-[312px]:px-2 max-[312px]:ml-1 ` }
                onClick={ (e) => {
                  e.preventDefault();
                  paginate(currentPage + 1);
                } }
